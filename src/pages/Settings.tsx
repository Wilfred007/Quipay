...
const AVAILABLE_PERMISSIONS = [
    ...
    { id: "treasury_approve", name: "Treasury Approver", description: "Can approve treasury transactions and proposals" },
    { id: "treasury_execute", name: "Treasury Execution", description: "Can execute approved treasury transactions" },
    { id: "admin_manage", name: "Admin Management", description: "Can manage team members and custom roles" },
];
...
const Settings: React.FC = () => {
    ...
    const [members] = useState<TeamMember[]>([
        { id: "1", name: "Organization Owner", address: "GCFX...ABC1", role: "Owner", status: "active" },
        { id: "2", name: "Alice Manager", address: "GDYQ...DEF2", role: "Treasury Approver", status: "active" },
        { id: "3", name: "Bob Accountant", address: "GAHU...GHI3", role: "Payroll Submitter", status: "pending" },
    ]);

    const [roles] = useState<CustomRole[]>([
        { id: "r1", name: "Payroll Submitter", description: "Responsible for initiating payroll runs", permissions: ["payroll_submit", "view_only"] },
        { id: "r2", name: "Treasury Approver", description: "Authorizes high-value treasury movements", permissions: ["treasury_approve", "view_only"] },
        { id: "r3", name: "View Only", description: "Stakeholders who only need read access", permissions: ["view_only"] },
    ]);

    const [auditLogs] = useState<AuditLog[]>([
        { id: "l1", timestamp: "2024-03-08 10:24:45", wallet: "GCFX...ABC1", action: "Created Stream", details: "Stream ID #104 for Worker GD...X22", status: "success" },
        { id: "l2", timestamp: "2024-03-08 09:12:10", wallet: "GDYQ...DEF2", action: "Approved Proposal", details: "Proposal #002: Upgrade Treasury Contract", status: "success" },
        { id: "l3", timestamp: "2024-03-07 18:30:22", wallet: "GAHU...GHI3", action: "Modified Role", details: "Updated Payroll Submitter permissions", status: "success" },
        { id: "l4", timestamp: "2024-03-07 15:45:00", wallet: "GCFX...ABC1", action: "Executed Withdrawal", details: "10,000 USDC to Operations Wallet", status: "success" },
    ]);
    ...
    const handleExportCSV = () => {
        ...
        const rows = auditLogs.map(log => [log.timestamp, log.wallet, log.action, log.details, log.status]);
        ...
        link.setAttribute("download", `quipay_audit_logs_${new Date().toISOString().split('T')[0]}.csv`);
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);

        setNotification({ message: "Audit logs exported successfully!", type: "success" });
    };
